...
pub struct RPCHandler {
    ...
    inbound_substreams:
        FnvHashMap<SubstreamId, (InboundSubstreamState, Option<delay_queue::Key>, Protocol)>,
    ...
    queued_outbound_items: FnvHashMap<SubstreamId, Vec<RPCCodedResponse>>,
...
pub enum InboundSubstreamState {
    ...
    /// responses.
    ResponseIdle(Box<InboundFramed<NegotiatedSubstream>>),
    /// The substream is attempting to shutdown.
    Closing(Box<InboundFramed<NegotiatedSubstream>>),
...
impl InboundSubstreamState {
    ...
    /// the outbound queue.
    pub fn close(&mut self, outbound_queue: &mut Vec<RPCCodedResponse>) {
...
fn apply_queued_responses(
    substream: InboundFramed<NegotiatedSubstream>,
    queued_outbound_items: &mut Option<&mut Vec<RPCCodedResponse>>,
